---
title: Páginas relacionadas ao primeiro aplicativo do App Builder
description: Links para páginas relacionadas na seção Primeiro aplicativo do App Builder
source-git-commit: 021098545e4666f43f02452358464701f30002fd
workflow-type: tm+mt
source-wordcount: '52'
ht-degree: 1%

---

# Links relacionados ao Primeiro aplicativo do App Builder

## Construtor de aplicativos — crie suas primeiras páginas relacionadas ao aplicativo

* [Visão geral](../app-builder/first-app/overview.md)
* [A variável ](../app-builder/first-app/env-file.md)
* [A variável ](../app-builder/first-app/app-config-yaml-file.md)
* [&quot;actions&quot;](../app-builder/first-app/actions-folder.md)
* [&quot;test&quot;](../app-builder/first-app/test-folder.md)
* [&quot;web-src&quot;](../app-builder/first-app/web-src-folder.md)
* [Testes locais](../app-builder/first-app/testing-locally.md)
* [Teste usando o URL do App Builder](../app-builder/first-app/testing-app-builder-url.md)
* [Conclusão](../app-builder/first-app/conclusion.md)
* [Código-fonte do módulo de amostra](https://github.com/magento/app-builder-samples){target="_blank"}
